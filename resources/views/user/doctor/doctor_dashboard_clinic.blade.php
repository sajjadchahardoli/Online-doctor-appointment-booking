    ...
    <div class="container ">
        <div class="row justify-content-center mt-5">
            <div class="col-md-8">
                ...
                <div class="card">
                    <div class="card-header card-header-color text-white">{{ __('ایجاد مطب') }}</div>

                    <div class="card-body">
                        <form method="POST" action="{{route('doctor.dashboard.clinic.store')}}">
                            @csrf
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input id="doctor_id" type="text" class="form-control @error('doctor_id') is-invalid @enderror"
                                           name="doctor_id" value="{{$id }}" required autocomplete="doctor_id" autofocus readonly>
...
{{--                                           value="{{ old('family') }}" required autocomplete="family" autofocus>--}}
        ...
        <div class="row justify-content-center mt-5 mb-5">
            <div class="col-md-8 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header card-header-color text-white">{{ __('لیست مطب‌ها') }}</div>
                    <div class="card-body table-responsive">
                        <table class="table table-hover">
                            ...
                            @foreach($clinics as $clinic)
                                @if($clinic->doctor_id  == $id)
                                    <tr>
                                        <td>{{$clinic->name}}</td>
                                        @switch($clinic->type)
                                            @case(1)
                                            @php
                                                $type = '<a href="" class="" style="color:black;">مطب</a>'
                                            @endphp
                                            @break

                                            @case(2)
                                            @php
                                                $type ='<a href="" class="badge " >کلینیک</a>';
                                            @endphp
                                            @break
                                            @default
                                        @endswitch
                                        ...
                                        <td>
                                                <a href="{{route('doctor.dashboard.clinic.destroy',$clinic->id)}}"
                                                   onclick="return confirm('ايا مايل به مطب هستید هستید')">
                                                    <label class="badge red-color text-white">حذف</label>
                                                </a>
                                                <a href="{{route('doctor.dashboard.clinic.edit',$clinic->id)}}" >
                                                    <label class="badge text-white blue-color">ویرایش</label>
                                                </a>
                                        </td>
