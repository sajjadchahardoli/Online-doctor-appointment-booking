                        ...
                        <li class="nav-item dropdown">
                            <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                @foreach($secretaries as $secretary)
                                    @if($secretary->id == $id)
                                        {{$secretary->name}} {{$secretary->family}}
                                    @endif
                                @endforeach
                            </a>
                            ...
                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                                ...
                                <a class="dropdown-item" href="{{route('secretary.dashboard.profile',$id)}}">
                                    {{ __('ویرایش پروفایل') }}
                                </a>
                                <a class="dropdown-item" href="{{route('secretary.dashboard.scheduling',$id)}}">
                                    {{ __('مدیریت زمانبندي') }}
                                </a>

                                <a class="dropdown-item" href="{{route('secretary.dashboard.reserve',$id)}}">
                                    {{ __('لیست بیماران') }}
                                </a>
                                            ...
                                            @if($id == $secretary->id)
                                                @foreach($clinics as $clinic)
                                                    @if($secretary->clinic_id == $clinic->id)
                                                        <option
                                                            value="{{$secretary->clinic_id}}">{{$clinic->name}}</option>
                                                    @endif
                                                @endforeach
                                            @endif
                                        @endforeach
                                    ...
                                    @foreach($secretaries as $secretary)
                                        @if($id == $secretary->id)
                                            <input id="doctor_id" type="text"
                                                   ...
                                                   name="doctor_id" value="{{$secretary->doctor_id}}" required
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-hover">
                            ...
                            @foreach($times as $time)
                                @foreach($secretaries as $secretary)
                                    @if($id == $secretary->id)
                                        @if($time->doctor_id  == $secretary->doctor_id)
                                            <tr>
                                                <td>{{$time->id}}</td>
                                                @foreach($clinics as $clinic)
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <a href="#">
                                                            <label class="badge badge-secondary disabled">حذف
                                                                غیرفعال</label>
                                                        </a>
